...
describe("Voting Tests", function () {
	async function deployVotingContract() {
		...
		// fix typescript issue: https://stackoverflow.com/questions/76458691/type-basecontract-omitcontractinterface-keyof-basecontract-is-missing-the
		const Voting = await (ethers as any).getContractFactory("Voting");
		// le contrat
	...
	describe("getter", function () {
		describe("getVoter", function () {
			it("should revert when not a voter", async function () {
				const { voting, owner, addr1 } = await loadFixture(
			...
			it("should return an unregistered voter", async function () {
				const { voting, owner, addr1, addr2 } = await loadFixture(
					deployVotingContract
				);
		...
		describe("getProposals", function () {
			it("should revert when not a voter", async function () {
				...
				await expect(voting.connect(owner).getProposals()).to.be.revertedWith(
					"You're not a voter"
				);
			});
	...
	describe("reset", function () {
		...
		describe("reset voters", function () {
			it("should revert when not owner", async function () {
				const { voting, addr1, addr2 } = await loadFixture(
					deployVotingContract
				);

				await expect(voting.connect(addr1).resetVoter(addr2.address))
					.to.be.revertedWithCustomError(voting, "OwnableUnauthorizedAccount")
					.withArgs(addr1.address);
			});
			it("should revert when workflow status is not RegisteringVoters", async function () {
				...
				await expect(
					voting.connect(owner).resetVoter(addr1.address)
				).to.be.revertedWith("Voting flow has already started");
			});
			it("should revert when voter has not beed registered yet", async function () {
				const { voting, owner, addr1 } = await loadFixture(
					deployVotingContract
				);
				...
				await expect(
					voting.connect(owner).resetVoter(addr1.address)
				).to.be.revertedWith("Not registered yet");
			});
			it("should reset voter when just registered", async function () {
				const { voting, owner, addr1 } = await loadFixture(
					deployVotingContract
				);
			...
			it("should reset voter when voted and workflow is restarting", async function () {
				const { voting, owner, addr1 } = await loadFixture(
					deployVotingContract
				);

				// add owner as voter to use getVoter
				...
				let [isRegistered, hasVoted, votedProposalId] = await voting
					.connect(owner)
					.getVoter(addr1);
				...
				// Reset the voter
				await voting.connect(owner).resetVoter(addr1.address);

				[isRegistered, hasVoted, votedProposalId] = await voting
					.connect(owner)
					.getVoter(addr1);
